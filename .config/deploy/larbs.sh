...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/yeetologist/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/yeetologist/dotfiles/master/.config/deploy/progs.csv"
...
adduserandpass() { \
	...
	usermod -a -G wheel,rfkill,sys,systemd-journal,video,storage,optical,disk,lp "$name" && mkdir -p /home/"$name" && chown "$name":wheel /home/"$name"
...
# Backlight
[ ! -f /etc/udev/rules.d/backlight.rules ] && printf 'RUN+="/bin/chgrp video /sys/class/backlight/intel_backlight/brightness"
...
%wheel ALL=(ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/wifi-menu,/usr/bin/mount,/usr/bin/umount,/usr/bin/paru -Syu --noconfirm,/usr/bin/systemctl restart NetworkManager,/usr/bin/rc-service NetworkManager restart,/usr/bin/loadkeys,/usr/bin/nnn,/usr/bin/sudoedit,/usr/bin/sysctl vm.drop_caches=3,/usr/bin/reflector,/usr/bin/find"
